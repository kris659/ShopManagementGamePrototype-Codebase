...
public class FirstPersonController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            {
                targetVelocity = rb.transform.TransformDirection(targetVelocity) * sprintSpeed;
            ...
            else
            {
                ...
                targetVelocity = rb.transform.TransformDirection(targetVelocity) * walkSpeed;
    ...
    {
        Vector3 origin = new Vector3(rb.transform.position.x, rb.transform.position.y + .5f, rb.transform.position.z);
        Vector3 direction = rb.transform.TransformDirection(Vector3.down);
        float distance = .75f;
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.rb = (Rigidbody)EditorGUILayout.ObjectField(new GUIContent("Player rigidbody"), fpc.rb, typeof(Rigidbody), true);

        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
        GUILayout.Label("Camera Setup", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));
        EditorGUILayout.Space();

        fpc.playerCamera = (GameObject)EditorGUILayout.ObjectField(new GUIContent("Camera", "Camera attached to the controller."), fpc.playerCamera, typeof(GameObject), true);
